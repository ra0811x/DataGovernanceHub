...
asset_name_mapping_bp = Blueprint(
    'asset_name_mapping',
    __name__
)
...
CONFIG_FILE = os.path.join(
    ...
    'config',
    'asset_name_mapping.json'
)
API_BASE = '/api/public-configs/asset-name-mappings'
MAPPING_SHEET_NAME = '资产名称映射'
SOURCE_COLUMN = '原资产名称'
TARGET_COLUMN = '标准资产名称'
ENABLED_COLUMN = '是否启用'
TEMPLATE_DOWNLOAD_NAME = '资产名称映射_导入模板.xlsx'


def _default_payload():
    return {
        'name': '公共配置',
        'key': 'asset_name_mapping',
        'title': '资产名称映射',
...
def load_mappings():
    ...
    try:
        ...
        if isinstance(data, dict):
            ...
            payload.update(data)
            payload['key'] = 'asset_name_mapping'
            payload['title'] = '资产名称映射'
            payload['description'] = '用于将源数据中的资产名称转换为统一口径的标准资产名称。'
...
@asset_name_mapping_bp.route(API_BASE, methods=['GET'])
def get_mappings():
    """获取全部资产名称映射。"""
    try:
        return jsonify({
...
@asset_name_mapping_bp.route(API_BASE, methods=['POST'])
def add_mapping():
    """新增资产名称映射。"""
    try:
...
@asset_name_mapping_bp.route(f'{API_BASE}/<int:mapping_id>', methods=['PUT'])
...
@asset_name_mapping_bp.route(f'{API_BASE}/<int:mapping_id>', methods=['DELETE'])
...
@asset_name_mapping_bp.route(f'{API_BASE}/batch', methods=['POST'])
def batch_import():
    """批量新增资产名称映射。"""
    try:
        req_data = request.get_json() or {}
...
@asset_name_mapping_bp.route(f'{API_BASE}/export', methods=['GET'])
def export_mappings():
    """导出资产名称映射为 Excel 文件。"""
    try:
        data = load_mappings()
        ...
        filename = f'资产名称映射_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
...
@asset_name_mapping_bp.route(f'{API_BASE}/import', methods=['POST'])
def import_mappings():
    """从 Excel 导入资产名称映射。"""
    try:
...
@asset_name_mapping_bp.route(f'{API_BASE}/template', methods=['GET'])
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            instructions = [
                ...
                ['1. 模板用途'],
                ['   本模板用于批量维护资产名称映射公共配置。'],
                ['   原资产名称填写源数据中的名称，标准资产名称填写统一口径后的名称。'],
                [''],
                ['2. 填写规范'],
                ['   原资产名称：必填。'],
                ['   标准资产名称：必填。'],
        ...
        response.headers['Content-Disposition'] = (
            f"attachment; filename=\"asset-name-mapping-template.xlsx\"; "
            f"filename*=UTF-8''{quote(TEMPLATE_DOWNLOAD_NAME)}"
        )
        return response
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
